 ...
 * @param V0 related to scale.  c=V0^{1/alpha} is the scale.
 ...
 */
double retstable_LD(double h, double alpha, RNG& r, double V0)
{
  ...
  if (h < 0 || alpha < 0 || alpha > 1 || V0 < 0) {
    fprintf(stderr, "Problem with parameter.\n");
    fprintf(stderr, "V0: %g; h: %g; alpha: %g\n", V0, h, alpha);
  }

  // report input
  // printf("V0: %g; h: %g; alpha: %g\n", V0, h, alpha);
  ...
  // for(int i = 0; i < n; i++) { /**< for each of the n required variates */
    ...
    double lambda_alpha = pow(h,alpha)*V0; 
    /**< Marius Hofert: work directly with lambda^alpha (numerically more stable for small alpha) */
	...
	// MYMY
	// printf("lambda^alpha=%g; b=%g; w1=%g; w2=%g, w3=%g\n", lambda_alpha, b, w1, w2, w3);
	...
	// printf("U=%g; Z=%g\n", U, Z);
      ...
      // MYMY
      // printf("Inner rejection sampling complete.\n");
      ...
      // MYMY
      // printf("m=%g; delta=%g; a1=%g; a2=%g; a3=%g\n", m, delta, a1, a2, a3);
