...
MatrixXd HmcSampler::sampleNext(bool returnTrace ) {
  ...
  while(2){
    ...
    while (1){
      ...
      #ifdef USE_R
      if (constraint_violations % 100 == 0) R_CheckUserInterrupt();
      #endif
    ...
    if (check >= 0){     //verify that we don't violate the constraints due to a numerical instability
      ...
      //   fprintf(stderr, "Cnst. violations: %i, initial check: %g\n", constraint_violations, initial_check);
      //   // fprintf(stderr, "Lots of cnst. violations: %i\n", constraint_violations);
      // }
      ...
      constraint_violations++;
      // if (constraint_violations % 1000 == 0)
      //	fprintf(stderr, "cnst. viol. %i\n", constraint_violations);
      if (constraint_violations == 1000)
	fprintf(stderr, "Reached 1000 constraint failures.  Not sure what HMC is doing.\n");
    }
...
double HmcSampler::verifyConstraints(const Eigen::VectorXd &v)
{
  return _verifyConstraints(v);
}
