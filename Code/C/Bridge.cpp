	...
	double tol, int max_iter, bool use_cg)
{
  ...
  return br.EM(beta, 1.0, ratio, alpha, lambda_max, tol, max_iter, use_cg);
}
		       ...
		       uint burn_in)
{
  ...
  cout << "Burn-in: " << burn_in << ", Num. Samples: " << M << "\n";
  ...
  cout << "Burn-in complete: " << (double)(end - start) / CLOCKS_PER_SEC
       << " sec. for " << burn_in << " iterations." << "\n";
		       ...
		       uint burn_in)
{
  ...
  cout << "Sampling complete: " << (double)(end - start) / CLOCKS_PER_SEC << " sec. for " << M << " iterations." << "\n";
			  ...
			  const int *burn_in)
{
  ...
  bridge_regression(beta, u, omega, y, X, *sig2_known, *tau_known, *alpha, (uint)(*burn_in));

  //  bridge_regression(beta, y, X, *sig2_known, *tau_known, *alpha, (uint)(*burn_in));
			 ...
			 const int *burn_in)
{
  ...
  bridge_regression(beta, u, omega, sig2, y, X,
		    *tau_known, *alpha,
		       ...
		       const int *burn_in)
{
  ...
  bridge_regression(beta, u, omega, sig2, tau, y, X,
		     *alpha,
		     *sig2_shape, *sig2_scale,
		     *nu_shape, *nu_rate,
		     *burn_in);
			      ...
			      uint burn_in)
{
  ...
  for(uint i = 0; i < burn_in; i++){
    br.sample_sig2(sig2[0], beta[0], sig2_shape, sig2_scale, r);  // Sample sig2.
    br.sample_lambda(lambda[0], beta[0], alpha, tau[0](0), RNG& r);
    br.sample_beta_stable(beta[0], lambda[0], alpha, sig2[0](0), tau[0](0), RNG& r);
  }
  ...
  for(uint i = 1; i < M; i++){
    br.sample_sig2(sig2[i], beta[i-1], sig2_shape, sig2_scale, r);  // Sample sig2.
    br.sample_lambda(lambda[i], beta[i-1], alpha, tau[i-1](0), RNG& r);
    br.sample_beta_stable(beta[i], lambda[i], alpha, sig2[i](0), tau[i-1](0), RNG& r);
  }
				...
				const int *burn_in)
{
  ...
  bridge_regression_stable(beta, lambda, sig2, y, X,
			   *tau_known, *alpha,
			   *sig2_shape, *sig2_scale,
			   (uint)*burn_in);
